...
export const handlejoinroombackend = async (roomid, email, username) => {
    ...
    if (!exsistingroom) {
        ...
        // This check is returning 409, but maybe user IS already in room
        if (exsistingroom.players.find((player) => player.email === email)) {
            // Instead of error, return success since the user is already in,
            return { message: "User already in the room", status: 200 };
        }
        const res = await fetchprofilepic(email);
        ...
        return { message: "joined the room", status: 200 };
    }
};
...
export const leaveroombackend = async (roomid, email) => {
    ...
    if (updatedRoom.players.length === 0) {
        ...
        return {
            message: "Player removed and empty room was deleted.",
            status: 200,
        };
    }
